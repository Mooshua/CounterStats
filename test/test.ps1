...
if ($USE_BUILDX -eq $True)
{
    ...
    iex "docker buildx build $docker_args --load --cache-from=type=local,src=cache --cache-to=type=local,dest=cache --file Dockerfile ../ -t counterstats-gdc:latest --build-arg CSGOVERSION=$csgo_version"
}
else 
{
    Write-Host "* Using Vanilla Docker"
    iex "docker build $docker_args --file Dockerfile ../ -t counterstats-gdc:latest --build-arg CSGOVERSION=$csgo_version"
}
Write-Host "* Done!"
...
$gamedatafiles = Get-ChildItem -Path ../gamedata -File -Name

$result_sum = 0

foreach ($gamedata in $gamedatafiles)
{
    docker run --rm counterstats-gdc:latest -f /test/gamedata/$gamedata -g csgo -e csgo -b /test/server/csgo/bin/server.so -w /test/server/csgo/bin/server.dll -x /test/server/bin/engine.so -y /test/server/bin/engine.dll -s /test/tools/gdc/symbols.txt > output.temp
...
docker run --name counterstats-exporter-dummy counterstats-gdc:latest > exporter.temp

foreach ($binary in $export_binaries)
{
    Write-Host "* Exporting $binary"
    docker cp counterstats-exporter-dummy:test/server/$binary ./bin
}

Write-Host "* Deleting Export Container"
docker rm counterstats-exporter-dummy
